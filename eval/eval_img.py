...
parser.add_argument("--root1", required=True, type=str,
                        help="root of the ground truth images")
parser.add_argument("--root2", required=True, type=str,
                        help="root of the predicted images")
...
if __name__ == '__main__':
    ...
    root1 = os.path.join(args.root1, "tf" + str(args.tf), "test")
    root2 = os.path.join(args.root2, "tf" + str(args.tf))

    fh = open(os.path.join(args.root1, "../test", "names.txt"))
    filenames = []
    for line in fh:
        line = line.strip("\r\n")
        filenames.append(line)

    fh = open(os.path.join(args.root1, "../../", "viewpoints.txt"))
    fh.readline()
    viewpoints = []
    for line in fh:
    ...
    for i in range(num_data):
        ...
        for j in range(num_vps):
            ...
            img2 = io.imread(os.path.join(root2, args.mode + filename, str(j) + ".png"))
    ...
    np.save(os.path.join(args.root2, "res", "ssim_" + args.mode + ".npy"), ssims)
    np.save(os.path.join(args.root2, "res", "emd_" + args.mode + ".npy"), emds)
